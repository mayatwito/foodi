...
def hungarian_best_restaurants(
    user_lat,
    user_lon,
    restaurants,
    top_k=5,
    ...
    # 🔹 מטריצת עלות: משתמש אחד × כל המסעדות
    cost_matrix = np.zeros((1, num_restaurants))

    for j, r in enumerate(restaurants):
        cost_matrix[0, j] = (
            distance_weight * r["distance_km"]
            + wait_weight * r["predicted_wait"]
        )

    # אלגוריתם הונגרי
    row_ind, col_ind = linear_sum_assignment(cost_matrix)

    # 🔹 המסעדה האופטימלית למשתמש
    best_idx = col_ind[0]

    # 🔹 דירוג מלא לפי אותה פונקציית עלות
    ranked = sorted(
        restaurants,
        key=lambda r: distance_weight * r["distance_km"] + wait_weight * r["predicted_wait"]
    )
...
def find_restaurant():
    ...
    ranked_results = hungarian_best_restaurants(
        lat,
        lon,
        results,
        top_k=5,
        distance_weight=3.0,  # מרחק חשוב יותר
        wait_weight=1.0
    )
